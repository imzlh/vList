...
class vp{
    // API
    $(cl){
        return this.container.getElementsByClassName('vpf_'+cl);
    }
    // 初始化元素
    set element(elem){
        // 初始化元素
        ...
        this.container.addEventListener('dblclick', function(e){
            if(e.target.classList.contains('vpf_video_box'))
                self.video.paused?self.video.play():self.video.pause();
        });
        var timer,mouse = function(){
            if(timer) clearTimeout(timer),timer = null;
            else self.e.layer.classList.add('vpf_active');
            timer = setTimeout(function(){
                self.e.layer.classList.remove('vpf_active');
                timer = null;
            },5000);
        };
        ...
        // this.e.bottom.addEventListener('mouseover',()=>self.e.layer.classList.add('vpf_force_active'));
        // this.e.bottom.addEventListener('mouseleave',()=>self.e.layer.classList.remove('vpf_force_active'));
        ...
        this.e.func.requestfs.parentElement.onclick = function(){
            if(document.fullscreen)
                ...
                // 使用妥协的方法全屏
                self.container.classList.contains('vpf_fullscreen')?
                    fullscreen(self.container.classList.remove('vpf_fullscreen')):
                    fullscreen(!self.container.classList.add('vpf_fullscreen'));
            }
        };
    ...
    // 整体初始化
    constructor (div){
        if(!(div instanceof HTMLElement || div.classList.contains('vpf_container')))
    ...
    alert(msg){
        ...
        alert.innerHTML = msg,alert.classList.add('vpf_active');
        this._alert_timer = setTimeout(function(){
            alert.classList.remove('vpf_active');
        },3000);
    }
    ...
    switch(src,subtitle){
        ...
        return new Promise(async function(rs,rj){
            if(video.src == src || (self.videoProxy && await self.videoProxy(src,self)))
                return rs();
            // 视频地址切换
